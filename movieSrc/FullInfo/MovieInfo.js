...
function MovieInfo(props) {
  ...
  const searchedItem = async id => {
    ...
    const {data: responseJson2} = await axios.get(
      'https://api.themoviedb.org/3/movie/' + id2 + '?api_key=' + apiKey,
    );
    responseJson = responseJson2;
  ...
  const getOverview = str => {
    var res = str.split(' ');
    var tot = '';
    res.forEach((element, i) => {
      if (i < 40) {
        //  ////console.log("s: ", i, element);
        tot = tot.concat(element);
        tot = tot.concat(' ');
      }
    });
    tot = tot.concat('...');
            ...
            <LinearGradient
              ...
              <View
                ...
                <View
                  ...
                  {category2serverData[0].tagline == '' ? null : (
                    <Text
                      style={{
                        color: 'white',
                        marginTop: 5,
                  ...
                  <Text
                    ...
                    {category2serverData[0].production_countries[0]
                      ? category2serverData[0].production_countries[0]
                          .iso_3166_1
                      : null}
                    ) &sdot; {getTime(category2serverData[0].runtime)}
                  </Text>
                </View>
              </View>
            </LinearGradient>
